/*
 * $Id: calendar-core.js 4338 2006-09-05 15:31:31Z slip $
 * The Masc DHTML Calendar
 *
 * Copyright (c) 2004 by Masc, Inc.
 * http://www.Masc.com
 ...
 *      cal.destroy();
 *   }
 *   var cal = new Masc.Calendar(1, new Date(), selFunc, closeFunc);
 * \endcode
 *
 ...
 * calendar using Masc.Calendar.create().
 *
 ...
 */
Masc.Calendar = function (firstDayOfWeek, dateStr, onSelected, onClose) {
	// member variables
	...
	this.minMonth = 0;
	this.maxMonth = 11;
	this.dateFormat = Masc.Calendar.i18n("DEF_DATE_FORMAT");
	this.ttDateFormat = Masc.Calendar.i18n("TT_DATE_FORMAT");
	this.historyDateFormat = "%B %d, %Y";
	this.isPopup = true;
	this.weekNumbers = true;
	this.noGrab = false;
	if (Masc.Calendar.prefs.fdow || (Masc.Calendar.prefs.fdow == 0)) {
		this.firstDayOfWeek = parseInt(Masc.Calendar.prefs.fdow, 10);
	}
	else {
		var fd = 0;
		if (typeof firstDayOfWeek == "number") {
			fd = firstDayOfWeek;
		} else if (typeof Masc.Calendar._FD == 'number') {
			fd = Masc.Calendar._FD;
		}
 ...
 */
Masc.Calendar._initSDN = function() {
	if (typeof Masc.Calendar._TT._SDN == "undefined") {
		// table of short day names
		if (typeof Masc.Calendar._TT._SDN_len == "undefined")
			Masc.Calendar._TT._SDN_len = 3;
		var ar = [];
		for (var i = 8; i > 0;) {
			ar[--i] = Masc.Calendar._TT._DN[i].substr(0, Masc.Calendar._TT._SDN_len);
		}
		Masc.Calendar._TT._SDN = ar;
		// table of short month names
		if (typeof Masc.Calendar._TT._SMN_len == "undefined")
			Masc.Calendar._TT._SMN_len = 3;
		ar = [];
		for (var i = 12; i > 0;) {
			ar[--i] = Masc.Calendar._TT._MN[i].substr(0, Masc.Calendar._TT._SMN_len);
		}
		Masc.Calendar._TT._SMN = ar;
	}
	if (typeof Masc.Calendar._TT.AMPM == "undefined") {
		Masc.Calendar._TT.AMPM = {am : "am", pm : "pm"};
	}
};
 ...
 */
Masc.Calendar.i18n = function(str, type) {
	var tr = '';
	if (!type) {
		// normal _TT request
		if (Masc.Calendar._TT)
			tr = Masc.Calendar._TT[str];
		if (!tr && Masc.Calendar._TT_en)
			tr = Masc.Calendar._TT_en[str];
	} else switch(type) {
	    case "dn"  : tr = Masc.Calendar._TT._DN[str];  break;
	    case "sdn" : tr = Masc.Calendar._TT._SDN[str]; break;
	    case "mn"  : tr = Masc.Calendar._TT._MN[str];  break;
	    case "smn" : tr = Masc.Calendar._TT._SMN[str]; break;
		case "ampm" : tr = Masc.Calendar._TT._AMPM[str]; break;
	}
	if (!tr) tr = "" + str;
	return tr;
};
...
Masc.Calendar._C = null;

/// preferences
Masc.Calendar.prefs = {
	fdow     : null,	/**< when NULL we will use the options passed at Masc.Calendar.setup */
 ...
 */
Masc.Calendar.savePrefs = function() {
	// FIXME: should we make the domain, path and expiration time configurable?
	// I guess these defaults are right though..
	Masc.Utils.writeCookie("ZP_CAL", Masc.Utils.makePref(this.prefs), null, '/', 30);
};

/**
 * Loads the preference cookie and merges saved prefs to Masc.Calendar.prefs.
 */
Masc.Calendar.loadPrefs = function() {
	var txt = Masc.Utils.getCookie("ZP_CAL"), tmp;
	if (txt) {
		tmp = Masc.Utils.loadPref(txt);
		if (tmp)
			Masc.Utils.mergeObjects(this.prefs, tmp);
	}
	// FIXME: DEBUG!
 ...
 */
Masc.Calendar._add_evs = function(el) {
	var C = Masc.Calendar;
	Masc.Utils.addEvent(el, "mouseover", C.dayMouseOver);
	Masc.Utils.addEvent(el, "mousedown", C.dayMouseDown);
	Masc.Utils.addEvent(el, "mouseout", C.dayMouseOut);
	if (Masc.is_ie)
		Masc.Utils.addEvent(el, "dblclick", C.dayMouseDblClick);
};

/**
 * \internal This function undoes what Masc.Calendar._add_evs did, therefore
 * unregisters the event handlers.
 *
 * @param el [HTMLElement] reference to your element.
 */
Masc.Calendar._del_evs = function(el) {
	var C = this;
	Masc.Utils.removeEvent(el, "mouseover", C.dayMouseOver);
	Masc.Utils.removeEvent(el, "mousedown", C.dayMouseDown);
	Masc.Utils.removeEvent(el, "mouseout", C.dayMouseOut);
	if (Masc.is_ie)
		Masc.Utils.removeEvent(el, "dblclick", C.dayMouseDblClick);
};
 ...
 */
Masc.Calendar.findMonth = function(el) {
...
Masc.Calendar.findHist = function(el) {
...
Masc.Calendar.findYear = function(el) {
 ...
 */
Masc.Calendar.showMonthsCombo = function () {
	var cal = Masc.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	for (var i = mc.firstChild; i; i = i.nextSibling) {
		var m = i.month;
		Masc.Utils.removeClass(i, "hilite");
		Masc.Utils.removeClass(i, "active");
		Masc.Utils.removeClass(i, "disabled");
		i.disabled = false;
		if ((min && m < cal.minMonth) ||
		    (max && m > cal.maxMonth)) {
			Masc.Utils.addClass(i, "disabled");
			i.disabled = true;
		}
		if (m == MM)
			Masc.Utils.addClass(cal.activeMonth = i, "active");
	}
	var s = mc.style;
	s.display = "block";
	if (cd.navtype < 0)
 ...
 * active calendar.
 */
Masc.Calendar.showHistoryCombo = function() {
	var cal = Masc.Calendar._C, a, h, i, cd, hc, s, tmp, div;
	if (!cal)
		return false;
	hc = cal.histCombo;
	while (hc.firstChild)
		hc.removeChild(hc.lastChild);
	if (Masc.Calendar.prefs.history) {
		a = Masc.Calendar.prefs.history.split(/,/);
		i = 0;
		while (tmp = a[i++]) {
			tmp = tmp.split(/\//);
			h = Masc.Utils.createElement("div");
			h.className = Masc.is_ie ? "label-IEfix" : "label";
 ...
 * otherwise.
 */
Masc.Calendar.showYearsCombo = function (fwd) {
	var cal = Masc.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	if (cal.hilitedYear) {
		Masc.Utils.removeClass(cal.hilitedYear, "hilite");
	}
	if (cal.activeYear) {
		Masc.Utils.removeClass(cal.activeYear, "active");
	}
	cal.activeYear = null;
 ...
 *
 * Also, the Masc.Calendar.cellClick() function is called, which further
 ...
 * @return false
 */
Masc.Calendar.tableMouseUp = function(ev) {
	var cal = Masc.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	var target = Masc.Utils.getTargetElement(ev);
	...
	ev || (ev = window.event);
	Masc.Utils.removeClass(el, "active");
	if (target == el || target.parentNode == el) {
		Masc.Calendar.cellClick(el, ev);
	}
	var mon = Masc.Calendar.findMonth(target);
	var date = null;
	if (mon) {
		if (!mon.disabled) {
		...
		var year = Masc.Calendar.findYear(target);
		if (year) {
			date = new Date(cal.date);
			...
			var hist = Masc.Calendar.findHist(target);
	...
	Masc.Utils.removeEvent(window.document, "mouseup", Masc.Calendar.tableMouseUp);
	Masc.Utils.removeEvent(window.document, "mouseover", Masc.Calendar.tableMouseOver);
	Masc.Utils.removeEvent(window.document, "mousemove", Masc.Calendar.tableMouseOver);
	cal._hideCombos();
	Masc.Calendar._C = null;
 ...
 * @return
 */
Masc.Calendar.tableMouseOver = function (ev) {
	var cal = Masc.Calendar._C;
	if (!cal) {
		return;
	}
	...
	var target = Masc.Utils.getTargetElement(ev);
	if (target == el || target.parentNode == el) {
		Masc.Utils.addClass(el, "hilite active");
		Masc.Utils.addClass(el.parentNode, "rowhilite");
	} else {
		if (typeof el.navtype == "undefined" ||
		    (el.navtype != 50 && ((el.navtype == 0 && !cal.histCombo) || Math.abs(el.navtype) > 2)))
			Masc.Utils.removeClass(el, "active");
		Masc.Utils.removeClass(el, "hilite");
		Masc.Utils.removeClass(el.parentNode, "rowhilite");
	}
	ev || (ev = window.event);
	if (el.navtype == 50 && target != el) {
		...
		if (cal.getDateStatus) { 
			...
			if (el.className.indexOf("ampm", 0) != -1) {
			   ...
			   if (old != range[i]) {
			      hour = (range[i] == Masc.Calendar.i18n("pm", "ampm")) ? ((date.getHours() == 0) ? (12) : (date.getHours() + 12)) : (date.getHours() - 12);
			   } else {
			      hour = date.getHours();
			   }
		...
		if (status == false) {
		   if ( !((!cal.time24) && (range[i] == Masc.Calendar.i18n("pm", "ampm")) && (hour > 23)) ) {
	...
	if (mon) {
		if (!mon.disabled) {
			if (mon.month != cal.date.getMonth()) {
				if (cal.hilitedMonth) {
					Masc.Utils.removeClass(cal.hilitedMonth, "hilite");
				}
				Masc.Utils.addClass(mon, "hilite");
		...
		if (year) {
			if (year.year != cal.date.getFullYear()) {
				...
				Masc.Utils.addClass(year, "hilite");
			...
			if (hist) {
				if (!hist.histDate.dateEqualsTo(cal.date)) {
					if (cal.hilitedHist) {
						Masc.Utils.removeClass(cal.hilitedHist, "hilite");
					}
					Masc.Utils.addClass(hist, "hilite");
 ...
 * @return false
 */
Masc.Calendar.tableMouseDown = function (ev) {
	if (Masc.Utils.getTargetElement(ev) == Masc.Utils.getElement(ev)) {
		return Masc.Utils.stopEvent(ev);
	}
};
 ...
 * @return false
 */
Masc.Calendar.calDragIt = function (ev) {
	ev || (ev = window.event);
	var cal = Masc.Calendar._C;
	if (!cal.disableDrag) {
		if (!(cal && cal.dragging)) {
		...
		st.left = L + "px";
		st.top = T + "px";
		Masc.Utils.setupWCH(cal.WCH, L, T);
	}
	return Masc.Utils.stopEvent(ev);
};
 ...
 * Masc.Calendar.hideShowCovered() which repaints as appropriate any
 ...
 * @return false
 */
Masc.Calendar.calDragEnd = function (ev) {
	var cal = Masc.Calendar._C;
	if (!cal) {
		return false;
	}
	cal.dragging = false;
	Masc.Utils.removeEvent(window.document, "mousemove", Masc.Calendar.calDragIt);
	Masc.Utils.removeEvent(window.document, "mouseover", Masc.Calendar.calDragIt);
	Masc.Utils.removeEvent(window.document, "mouseup", Masc.Calendar.calDragEnd);
	Masc.Calendar.tableMouseUp(ev);
	cal.hideShowCovered();
};
//@}
 ...
 * Masc.Calendar.tableMouseOver() event handler on the document.  If the
 ...
 * @return false
 */
Masc.Calendar.dayMouseDown = function(ev) {
	var canDrag = true;
	var el = Masc.Utils.getElement(ev);
	...
	if (el.navtype != 300) {
		if (el.navtype == 50) {
			...
			if (canDrag) {Masc.Utils.addEvent(window.document, "mousemove", Masc.Calendar.tableMouseOver);}
		} else {
			if (((el.navtype == 201) || (el.navtype == 202)) && (cal.timeInterval > 30) && (el.timePart.className.indexOf("minute", 0) != -1)) {canDrag = false;}
			if (canDrag) {Masc.Utils.addEvent(window.document, Masc.is_ie5 ? "mousemove" : "mouseover", Masc.Calendar.tableMouseOver);}
		}
		if (canDrag) {Masc.Utils.addClass(el, "hilite active");}
		Masc.Utils.addEvent(window.document, "mouseup", Masc.Calendar.tableMouseUp);
	} else if (cal.isPopup) {
		cal._dragStart(ev);
	} else {
		Masc.Calendar._C = null;
	...
	if (el.navtype == -1 || el.navtype == 1) {
		...
		cal.timeout = setTimeout("Masc.Calendar.showMonthsCombo()", 250);
	} else if (el.navtype == -2 || el.navtype == 2) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout((el.navtype > 0) ? "Masc.Calendar.showYearsCombo(true)" : "Masc.Calendar.showYearsCombo(false)", 250);
	} else if (el.navtype == 0 && Masc.Calendar.prefs.history) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout("Masc.Calendar.showHistoryCombo()", 250);
	} else {
		cal.timeout = null;
	}
	return Masc.Utils.stopEvent(ev);
 ...
 */
Masc.Calendar.dayMouseDblClick = function(ev) {
	Masc.Calendar.cellClick(Masc.Utils.getElement(ev), ev || window.event);
	if (Masc.is_ie)
 ...
 * @return false
 */
Masc.Calendar.dayMouseOver = function(ev) {
	var el = Masc.Utils.getElement(ev),
		caldate = el.caldate;
	//BEGIN: fix for the extra information bug in IE
	while (!el.calendar) {
	...
	if (Masc.Utils.isRelated(el, ev) || Masc.Calendar._C || el.className.indexOf("disabled") != -1 || el.className.indexOf("true") != -1) {
		return false;
	}
	if (el.ttip) {
	...
	if (el.navtype != 300) {
		...
		if (!((cal.timeInterval == null) || (el.className.indexOf("ampm", 0) != -1) || ((cal.timeInterval < 60) && (el.className.indexOf("hour", 0) != -1))) && (el.navtype == 50)) {return Masc.Utils.stopEvent(ev);}
		if (((el.navtype == 201) || (el.navtype == 202)) && (cal.timeInterval > 30) && (cel.className.indexOf("minute", 0) != -1)) {return Masc.Utils.stopEvent(ev);}
		Masc.Utils.addClass(el, "hilite");
		if (caldate) {
			Masc.Utils.addClass(el.parentNode, "rowhilite");
		}
	}
 ...
 * @return false
 */
Masc.Calendar.dayMouseOut = function(ev) {
	var el = Masc.Utils.getElement(ev);
	//BEGIN: fix for the extra information bug in IE
	while (!el.calendar) {
		el = el.parentNode;
	...
	if (el.caldate)
		Masc.Utils.removeClass(el.parentNode, "rowhilite");
	if (el.calendar)
		el.calendar.showHint(Masc.Calendar.i18n("SEL_DATE"));
	return Masc.Utils.stopEvent(ev);
};
 ...
 * @param ev [Event] the event object
 */
Masc.Calendar.cellClick = function(el, ev) {
	var cal = el.calendar;
	var closing = false;
	var newdate = false;
	var date = null;
	...
	if (typeof el.navtype == "undefined") {
		if (cal.currentDateEl) {
			Masc.Utils.removeClass(cal.currentDateEl, "selected");
			Masc.Utils.addClass(el, "selected");
		...
		switch (el.navtype) {
		    case 400:
			Masc.Utils.removeClass(el, "hilite");
			var text = Masc.Calendar.i18n("ABOUT");
			if (typeof text != "undefined") {
				text += cal.showsTime ? Masc.Calendar.i18n("ABOUT_TIME") : "";
			} else {
				// FIXME: this should be removed as soon as lang files get updated!
				text = "Help and about box text is not translated into this language.\n" +
					"If you know this language and you feel generous please update\n" +
					"the corresponding file in \"lang\" subdir to match calendar-en.js\n" +
					"and send it back to <support@Masc.com> to get it into the distribution  ;-)\n\n" +
					"Thank you!\n" +
					"http://www.Masc.com\n";
			}
			alert(text);
			...
			Masc.Calendar.prefs.fdow = cal.firstDayOfWeek;
			Masc.Calendar.savePrefs();
			if (cal.onFDOW)
		...
		//ALLOWED TIME CHECK
			if (cal.getDateStatus) { //Current time is changing, check with the callback to see if it's in range
				...
				if (el.className.indexOf("ampm", 0) != -1) {
					minute = date.getMinutes(); // minutes didn't change
					// if the "ampm" value has changed we need to correct hours (add 12 or exclude 12 or set it to zero)
					hour = (range[i] == Masc.Calendar.i18n("pm", "ampm")) ? ((date.getHours() == 12) ? (date.getHours()) : (date.getHours() + 12)) : (date.getHours() - 12);
					...
					if ( cal.getDateStatus && cal.getDateStatus(new_date, date.getFullYear(), date.getMonth(), date.getDate(), parseInt(hour, 10), parseInt(minute, 10)) ) {
					   var dirrect;
					   if (range[i] == Masc.Calendar.i18n("pm", "ampm")) {
					      dirrect = -5;
					   } else {
					      dirrect = 5;
					   }
 ...
 * Masc.Calendar.prototype._init().  Therefore, by separating these 2
 ...
 * @param _par
 */
Masc.Calendar.prototype.create = function (_par) {
	var parent = null;
	if (! _par) {
	...
	var table = Masc.Utils.createElement("table");
	this.table = table;
	table.cellSpacing = 0;
	table.cellPadding = 0;
	table.calendar = this;
	Masc.Utils.addEvent(table, "mousedown", Masc.Calendar.tableMouseDown);

	var div = Masc.Utils.createElement("div");
	this.element = div;
	...
	var hh = function (text, cs, navtype) {
		cell = Masc.Utils.createElement("td", row);
		cell.colSpan = cs;
		cell.className = "button";
		if (Math.abs(navtype) <= 2)
			cell.className += " nav";
		Masc.Calendar._add_evs(cell);
		cell.calendar = cal;
	...
	var thead = Masc.Utils.createElement("thead", table);
	if (this.numberMonths == 1) {
		this.title = thead;
	}
	row = Masc.Utils.createElement("tr", thead);
	if (this.helpButton) {
		hh("?", 1, 400).ttip = Masc.Calendar.i18n("INFO");
	} else {
		cell = Masc.Utils.createElement("td", row);
		cell.colSpan = 1;
		cell.className = "button";
	...
	if (this.isPopup) {
		if (!this.disableDrag) {
			this.title.ttip = Masc.Calendar.i18n("DRAG_TO_MOVE");
			this.title.style.cursor = "move";
		}
		hh("&#x00d7;", 1, 200).ttip = Masc.Calendar.i18n("CLOSE");
	} else {
		cell = Masc.Utils.createElement("td", row);
		cell.colSpan = 1;
		cell.className = "button";
	...
	row = Masc.Utils.createElement("tr", thead);
	this._nav_py = hh("&#x00ab;", 1, -2);
	this._nav_py.ttip = Masc.Calendar.i18n("PREV_YEAR");
	this._nav_pm = hh("&#x2039;", 1, -1);
	this._nav_pm.ttip = Masc.Calendar.i18n("PREV_MONTH");
	this._nav_now = hh(Masc.Calendar.i18n("TODAY"), title_length - 2, 0);
	this._nav_now.ttip = Masc.Calendar.i18n("GO_TODAY");
	this._nav_nm = hh("&#x203a;", 1, 1);
	this._nav_nm.ttip = Masc.Calendar.i18n("NEXT_MONTH");
	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny.ttip = Masc.Calendar.i18n("NEXT_YEAR");
	...
	for (var l = 1; l <= rowsOfMonths; ++l) {
		...
		if (Masc.is_opera) {thead.style.display = "table-row-group";}
		if (this.numberMonths != 1) {
			row = Masc.Utils.createElement("tr", thead);
			var title_length = 5;
	...
	// day names
		...
		for (k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
			if (this.weekNumbers) {
				...
				cell.appendChild(window.document.createTextNode(Masc.Calendar.i18n("WK")));
				if (k > 1) {
					Masc.Utils.addClass(cell, "month-left-border");
				}
				var cal_wk = Masc.Calendar.i18n("WK")
					if (cal_wk == null) {
		...
		var tbody = Masc.Utils.createElement("tbody", table);
		this.tbody[l] = tbody;
		
		for (i = 6; i > 0; --i) {
			//creating a row of days for all the months in the row
			row = Masc.Utils.createElement("tr", tbody);
			for (k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
				if (this.weekNumbers) {
					cell = Masc.Utils.createElement("td", row);
	...
	var tfoot = Masc.Utils.createElement("tfoot", table);

	if (this.showsTime) {
		row = Masc.Utils.createElement("tr", tfoot);
		row.className = "time";
		...
		cell.innerHTML = Masc.Calendar.i18n("TIME") || "&nbsp;";
		...
		(function() {
			function makeTimePart(className, init, range_start, range_end) {
				...
				if (range_end) {
					...
					table = Masc.Utils.createElement("table", cell);
					table.cellSpacing = table.cellPadding = 0;
					if (className == "hour")
						table.align = "right";
					table.className = "calendar-time-scroller";
					tbody = Masc.Utils.createElement("tbody", table);
					tr    = Masc.Utils.createElement("tr", tbody);
					tr2   = Masc.Utils.createElement("tr", tbody);
				} else
					tr = row;
				part = Masc.Utils.createElement("td", tr);
				part.className = className;
				part.appendChild(window.document.createTextNode(init));
				part.calendar = cal;
				part.ttip = Masc.Calendar.i18n("TIME_PART");
				part.navtype = 50;
				part._range = [];
				if (!range_end)
					...
					var up = Masc.Utils.createElement("td", tr);
					up.className = "up";
					up.navtype = 201;
					up.calendar = cal;
					up.timePart = part;
					if (Masc.is_khtml)
						up.innerHTML = "&nbsp;";
					Masc.Calendar._add_evs(up);

					var down = Masc.Utils.createElement("td", tr2);
					down.className = "down";
					down.navtype = 202;
					down.calendar = cal;
					down.timePart = part;
					if (Masc.is_khtml)
						down.innerHTML = "&nbsp;";
					Masc.Calendar._add_evs(down);
				}
				Masc.Calendar._add_evs(part);
				return part;
			};
			...
			var AP = null;
			if (t12) {
				AP = makeTimePart("ampm", pm ? Masc.Calendar.i18n("pm", "ampm") : Masc.Calendar.i18n("am", "ampm"), [Masc.Calendar.i18n("am", "ampm"), Masc.Calendar.i18n("pm", "ampm")]);
				AP.className += " button";
			} else
				Masc.Utils.createElement("td", row).innerHTML = "&nbsp;";
			...
			//END OF ALLOWED TIME CHECK
				...
				if (t12)
				   AP.firstChild.data = pm ? Masc.Calendar.i18n("pm", "ampm") : Masc.Calendar.i18n("am", "ampm");
			};
	...
	cell = hh(Masc.Calendar.i18n("SEL_DATE"), this.weekNumbers ? (8 * this.numberMonths) : (7 * this.numberMonths), 300);
	cell.className = "ttip";
	if (this.isPopup && !this.disableDrag) {
		cell.ttip = Masc.Calendar.i18n("DRAG_TO_MOVE");
		cell.style.cursor = "move";
	}
	this.tooltips = cell;

	div = this.monthsCombo = Masc.Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 0; i < 12; ++i) {
		var mn = Masc.Utils.createElement("div");
		mn.className = Masc.is_ie ? "label-IEfix" : "label";
		mn.month = i;
		mn.appendChild(window.document.createTextNode(Masc.Calendar.i18n(i, "smn")));
		div.appendChild(mn);
	}

	div = this.yearsCombo = Masc.Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 12; i > 0; --i) {
		var yr = Masc.Utils.createElement("div");
		yr.className = Masc.is_ie ? "label-IEfix" : "label";
		yr.appendChild(window.document.createTextNode(""));
		div.appendChild(yr);
	}

	div = this.histCombo = Masc.Utils.createElement("div", this.element);
 ...
 * @return false
 */
Masc.Calendar._keyEvent = function(ev) {
	if (!window.calendar) {
		return false;
	}
	(Masc.is_ie) && (ev = window.event);
	var cal = window.calendar;
	var act = (Masc.is_ie || ev.type == "keypress");
	var K = ev.keyCode;
	var date  = new Date(cal.date);
	if (ev.ctrlKey) {
		switch (K) {
		    case 37: // KEY left
			act && Masc.Calendar.cellClick(cal._nav_pm);
			break;
		    case 38: // KEY up
			act && Masc.Calendar.cellClick(cal._nav_py);
			break;
		    case 39: // KEY right
			act && Masc.Calendar.cellClick(cal._nav_nm);
			break;
		    case 40: // KEY down
			act && Masc.Calendar.cellClick(cal._nav_ny);
			break;
		    default:
			return false;
		}
		...
		Masc.Calendar.cellClick(cal._nav_now);
		break;
	    case 27: // KEY esc
		...
		if (act) {
			//FIX for Enter key!
			Masc.Calendar.cellClick(cal.currentDateEl);
		}
		break;
	    default:
		return false;
 ...
 * @return
 */
Masc.Calendar.prototype._init = function (firstDayOfWeek, date, last) {
	var
		today = new Date(),
		TD = today.getDate(),
	...
	var MN = Masc.Calendar.i18n(month, "smn");
	var weekend = Masc.Calendar.i18n("WEEKEND");
	...
	for (var l = 1; l <= rowsOfMonths; ++l) {
		...
		for (var i = 7; --i > 0; row = row.nextSibling) {
			...
			for (var k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
				...
				for (j = 7; cell && (iday = date.getDate()) && (j > 0); date.setDate(iday+1), ((date.getDate() == iday) ? (date.setHours(1) && date.setDate(iday + 1)) : (false)), cell = cell.nextSibling, --j) {
					...
					if ((j == 1) && (k != this.monthsInRow)) {
						Masc.Utils.addClass(cell, "month-right-border");
					}
					if (this.vertical) {
					...
					if (!cell.disabled) {
						...
						if (dyear == TY && dmonth == TM && iday == TD) {
							...
							cell.ttip += Masc.Calendar.i18n("PART_TODAY");
						}
			...
			if ((i == 1) && (l < rowsOfMonths)) {
				...
				cell = row.firstChild;
				while (cell != null) {
					Masc.Utils.addClass(cell, "month-bottom-border");
					cell = cell.nextSibling;
				}
			}
	...
	//this.title.firstChild.data = Masc.Calendar.i18n(month, "mn") + ", " + year;
	//filling of all titles for the months
	if (this.numberMonths == 1) {
		this.title.innerHTML = Masc.Calendar.i18n(month, "mn") + ", " + year;
      ...
      if (this.params && this.params.titleHtml)
         if (typeof this.params.titleHtml == 'function')
            this.title.innerHTML = this.params.titleHtml(Masc.Calendar.i18n(month, "mn") + ", " + year, month, year)
         else
           this.title.innerHTML = this.params.titleHtml
		...
		for (var l = 1; l <= rowsOfMonths; ++l) {
			for (var k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
				...
				this.titles[l][k].innerHTML = Masc.Calendar.i18n(validMonth, "mn") + ", " + validYear;
			}
		}
	}	
	this.onSetTime();
 ...
 */
Masc.Calendar.prototype._initMultipleDates = function() {
	if (this.multiple) {
 ...
 */
Masc.Calendar.prototype._toggleMultipleDate = function(date) {
	if (this.multiple) {
		...
		if (cell) {
			...
			if (!d) {
				Masc.Utils.addClass(cell, "selected");
				this.multiple[ds] = date;
			} else {
				Masc.Utils.removeClass(cell, "selected");
 ...
 */
Masc.Calendar.prototype.setDateToolTipHandler = function (unaryFunction) {
 ...
 */
Masc.Calendar.prototype.setDate = function (date, justInit) {
	...
	if (!date.equalsTo(this.date)) {
		...
		if (year == this.minYear && m < this.minMonth)
			this.showHint("<div class='error'>" + Masc.Calendar.i18n("E_RANGE") + " »»»</div>");
 ...
 */
Masc.Calendar.prototype.showHint = function(text) {
 ...
 */
Masc.Calendar.prototype.reinit = function() {
	this._init(this.firstDayOfWeek, this.date);
};
 ...
 */
Masc.Calendar.prototype.refresh = function() {
 ...
 */
Masc.Calendar.prototype.setFirstDayOfWeek = function (firstDayOfWeek) {
 ...
 */
Masc.Calendar.prototype.setDateStatusHandler = Masc.Calendar.prototype.setDisabledHandler = function (unaryFunction) {
 ...
 */
Masc.Calendar.prototype.setRange = function (A, Z) {
	var m,
		a = Math.min(A, Z),
		z = Math.max(A, Z);
 ...
 */
Masc.Calendar.prototype.setMultipleDates = function(multiple) {
 ...
 */
Masc.Calendar.prototype.submitFlatDates = function()
{
	if (typeof this.params.flatCallback == "function") {
	   //Specify the pre-set sorting so that Masc.Utils will sort the multiple dates accordingly. 
	   //Default to "asc" if it does not equal to "asc" or "desc".
	   Masc.Utils.sortOrder = (this.sortOrder!="asc" && this.sortOrder!="desc" && this.sortOrder!="none") ? "none" : this.sortOrder;
			
	   if ( this.multiple && (Masc.Utils.sortOrder != "none") ) {
			...
			for (var i in this.multiple) {
			    ...
			    dateArray.sort(Masc.Utils.compareDates);
			}
 ...
 * calendar.dateFormat.
 */
Masc.Calendar.prototype.callHandler = function () {
	if (this.onSelected) {
 ...
 */
Masc.Calendar.prototype.updateHistory = function () {
	var a, i, d, tmp, s, str = "", len = Masc.Calendar.prefs.hsize - 1;
	if (Masc.Calendar.prefs.history) {
		a = Masc.Calendar.prefs.history.split(/,/);
		i = 0;
		while (i < len && (tmp = a[i++])) {
	...
	Masc.Calendar.prefs.history = this.date.print("%Y/%m/%d/%H/%M") + str;
	Masc.Calendar.savePrefs();
};
 ...
 */
Masc.Calendar.prototype.callCloseHandler = function () {
	if (this.dateClicked) {
		this.updateHistory();
	}
	if (this.onClose) {
...
Masc.Calendar.prototype.destroy = function () {
	this.hide();		// this also removes keyboard events :-\
	Masc.Utils.destroy(this.element);
	Masc.Utils.destroy(this.WCH);
	Masc.Calendar._C = null;
 ...
 */
Masc.Calendar.prototype.reparent = function (new_parent) {
	var el = this.element;
 ...
 */
Masc.Calendar._checkCalendar = function(ev) {
	if (!window.calendar) {
		return false;
	}
	var el = Masc.is_ie ? Masc.Utils.getElement(ev) : Masc.Utils.getTargetElement(ev);
 ...
 */
Masc.Calendar.prototype.updateWCH = function(other_el) {
	Masc.Utils.setupWCH_el(this.WCH, this.element, other_el);
};
 ...
 */
Masc.Calendar.prototype.show = function () {
	var rows = this.table.getElementsByTagName("tr");
	for (var i = rows.length; i > 0;) {
		var row = rows[--i];
		Masc.Utils.removeClass(row, "rowhilite");
		var cells = row.getElementsByTagName("td");
		for (var j = cells.length; j > 0;) {
			var cell = cells[--j];
			Masc.Utils.removeClass(cell, "hilite");
			Masc.Utils.removeClass(cell, "active");
		}
	}
	...
	if (this.isPopup) {
		...
		if (!this.noGrab) {
			Masc.Utils.addEvent(window.document, "keydown", Masc.Calendar._keyEvent);
			Masc.Utils.addEvent(window.document, "keypress", Masc.Calendar._keyEvent);
			Masc.Utils.addEvent(window.document, "mousedown", Masc.Calendar._checkCalendar);
		}
	}
	this.hideShowCovered();
};
 ...
 * mousedown.
 */
Masc.Calendar.prototype.hide = function () {
	if (this.isPopup) {
		Masc.Utils.removeEvent(window.document, "keydown", Masc.Calendar._keyEvent);
		Masc.Utils.removeEvent(window.document, "keypress", Masc.Calendar._keyEvent);
		Masc.Utils.removeEvent(window.document, "mousedown", Masc.Calendar._checkCalendar);
	}
	this.element.style.display = "none";
	Masc.Utils.hideWCH(this.WCH);
	this.hidden = true;
	this.hideShowCovered();
};
 ...
 */
Masc.Calendar.prototype.showAt = function (x, y) {
 ...
 */
Masc.Calendar.prototype.showAtElement = function (el, opts) {
	var self = this;
	var p = Masc.Utils.getAbsolutePos(el);
 ...
 */
Masc.Calendar.prototype.setDateFormat = function (str) {
	this.dateFormat = str;
};

/** Customizes the tooltip date format.  See
 * Masc.Calendar.prototype.setDateFormat() for a description of the \em str
 * format.
 *
 * @param str [string] the "tooltip" date format
 */
Masc.Calendar.prototype.setTtDateFormat = function (str) {
 ...
 */
Masc.Calendar.prototype.parseDate = function (str, fmt) {
	// Konqueror
	if (!str)
 ...
 */
Masc.Calendar.prototype.hideShowCovered = function () {
	if (!Masc.is_ie5)
		return;
	var self = this;
	...
	for (var k = tags.length; k > 0; ) {
		...
		for (var i = ar.length; i > 0;) {
			...
			p = Masc.Utils.getAbsolutePos(cc);
			var CX1 = p.x;
 ...
 */
Masc.Calendar.prototype._displayWeekdays = function () {
	...
	for (k = 1; (k <= this.monthsInRow) && (cell); ++k) {
		for (var i = 0; i < 7; ++i) {
			...
			if ((!this.disableFdowClick) && ((this.params && this.params.fdowClick) || i)) {
				if (Masc.Calendar.i18n("DAY_FIRST") != null) {
					cell.ttip = Masc.Calendar.i18n("DAY_FIRST").replace("%s", Masc.Calendar.i18n(realday, "dn"));
				}
				cell.navtype = 100;
				cell.calendar = this;
				cell.fdow = realday;
			...
			if ((weekend != null) && (weekend.indexOf(realday.toString()) != -1)) {
							Masc.Utils.addClass(cell, "weekend");
			}
			cell.innerHTML = Masc.Calendar.i18n((i + fdow) % 7, "sdn");
			cell = cell.nextSibling;
		}
 ...
 */
Masc.Utils.compareDates = function(date1, date2)
{
	if (Masc.Calendar.prefs.sortOrder == "asc")
		return date1 - date2;
...
Masc.Calendar.prototype._hideCombos = function () {
...
Masc.Calendar.prototype._dragStart = function (ev) {
	ev || (ev = window.event);
	if (this.dragging) {
		return;
	}
	...
	Masc.Utils.addEvent(window.document, "mousemove", Masc.Calendar.calDragIt);
	Masc.Utils.addEvent(window.document, "mouseover", Masc.Calendar.calDragIt);
	Masc.Utils.addEvent(window.document, "mouseup", Masc.Calendar.calDragEnd);
};
