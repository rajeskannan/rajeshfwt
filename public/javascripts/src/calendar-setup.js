/* 
 * $Id: calendar-setup.js 4322 2006-09-04 08:49:33Z shacka $
...
Masc.Setup = function () {};
Masc.Setup.test = true;

/**
 *  This function "patches" an input field (or other element) to use a calendar
 *  widget for date selection.
 *
 *  Note that you can use the Masc DHTML Calendar Wizard and generate the code
 ...
 */
Masc.Calendar.setup = function (params) {
	paramsList = ["id"];
	...
	params.id = Masc.Utils.generateID("calendar");
	...
	if (params.saveDate) { //If saveDate is on We're saving the date in a cookie
		...
		var newdate = Masc.Utils.getCookie(cookieName);
	...
	for(var ii in params) {
		if (Masc.Utils.arrIndexOf(paramsList, ii) == -1) {
	...
	function onSelect(cal) {
		...
		if (p.saveDate) { //save date in cookie
			...
			Masc.Utils.writeCookie(cookieName, p.inputField.value, null, '/', p.saveDate);
		} 
	};
	...
	if (params.flat != null) {
		...
		var cal = new Masc.Calendar(params.firstDay, params.date, params.onSelect || onSelect);
		cal.id = params.id;
	...
	triggerEl["on" + params.eventName] = function() {
		...
		if (!(cal && params.cache)) {
			window.calendar = cal = new Masc.Calendar(params.firstDay,
							     params.date,
